...
def train(epoch, train_loader, model, optimizer, use_mlboard=True):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % 500 == 0:
            ...
            if use_mlboard:
                mlboard.update_task_info({'train_loss': loss_data})
#
# A simple test procedure to measure STN the performances on MNIST.
#


def test(test_loader, model, use_mlboard=True):
    model.eval()
    test_loss = 0
    correct = 0
...
def visualize_stn(test_loader, model, out_dir):
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train(epoch, train_loader, model, optimizer, use_mlboard=not args.skip_mlboard)
